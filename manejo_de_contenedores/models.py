...
class contenedor(models.Model):
	...
	total = float(self.peso_salida_completo - ((self.peso_ingreso_completo - self.peso_ingreso_cabezal) + self.peso_salida_cabezal))

	if "New" not in str(self.id):
		if len(order_line) == 0 and self.peso_salida_completo > 0 and self.peso_ingreso_completo > 0 and self.peso_ingreso_cabezal > 0 and self.peso_salida_cabezal > 0:
			self.order_line.create({'name':str(res.name), 'order_id':self.id, 'product_uom_qty':float(total), 'product_id': int(res.id)})
